...
const profileFields = z.object({ phone: z.string().trim().max(40).nullable().optional(), street: z.string().trim().max(160).nullable().optional(), postalCode: z.string().trim().max(20).nullable().optional(), city: z.string().trim().max(100).nullable().optional(), teacherCode: z.string().trim().max(40).nullable().optional(), teachingLevels: z.array(teachingLevelSchema).max(30).optional(), notes: z.string().trim().max(5000).nullable().optional(), active: z.boolean().optional(), ratePerLesson: z.number().min(0).max(10000).nullable().optional() });
...
export async function POST(request: Request) {
  try {
    ...
    const teacher = await sql.begin(async (transaction) => {
      ...
      await transaction`INSERT INTO teacher_profiles (user_id, teacher_code, phone, street, postal_code, city, notes, active, rate_per_lesson) VALUES (${user.id}, ${input.teacherCode ?? null}, ${input.phone ?? null}, ${input.street ?? null}, ${input.postalCode ?? null}, ${input.city ?? null}, ${input.notes ?? null}, ${input.active ?? true}, ${input.ratePerLesson ?? null})`;
