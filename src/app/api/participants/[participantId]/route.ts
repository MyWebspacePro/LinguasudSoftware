...
export async function GET(_request: Request, context: { params: Promise<{ participantId: string }> }) {
  try {
    ...
    const [enrollments, history, notes] = await Promise.all([
      sql`SELECT e.*, json_build_object('id', c.id, 'code', c.code, 'language', c.language, 'level', c.level, 'status', c.status, 'teacher', json_build_object('id', t.id, 'name', t.name, 'email', t.email), 'standardRoom', CASE WHEN r.id IS NULL THEN NULL ELSE json_build_object('id', r.id, 'name', r.name, 'capacity', r.capacity, 'location', json_build_object('id', loc.id, 'name', loc.name, 'address', loc.address)) END, 'schedules', COALESCE((SELECT json_agg(json_build_object('id', cs.id, 'weekday', cs.weekday, 'startTime', cs.start_time, 'durationMinutes', cs.duration_minutes) ORDER BY cs.weekday, cs.start_time) FROM course_schedules cs WHERE cs.course_id = c.id), '[]')) AS course, COALESCE((SELECT json_agg(json_build_object('id', l.id, 'lessonId', l.id, 'startsAt', l.starts_at, 'status', l.status, 'roomId', l.room_id, 'attendanceStatus', a.status, 'confirmedAt', a.confirmed_at) ORDER BY l.starts_at) FROM lessons l LEFT JOIN attendance a ON a.lesson_id = l.id AND a.enrollment_id = e.id WHERE l.course_id = e.course_id), '[]') AS lesson_history, COALESCE((SELECT json_build_object('present', count(*) FILTER (WHERE a.status = 'present')::int, 'excusedPending', count(*) FILTER (WHERE a.status = 'excused_pending')::int, 'excused', count(*) FILTER (WHERE a.status = 'excused')::int, 'unexcused', count(*) FILTER (WHERE a.status = 'unexcused')::int, 'online', count(*) FILTER (WHERE a.status = 'online')::int, 'trial', count(*) FILTER (WHERE a.status = 'trial')::int) FROM attendance a WHERE a.enrollment_id = e.id), json_build_object('present', 0, 'excusedPending', 0, 'excused', 0, 'unexcused', 0, 'online', 0, 'trial', 0)) AS attendance_summary, COALESCE((SELECT json_agg(json_build_object('id', pause.id, 'startsOn', pause.starts_on, 'endsOn', pause.ends_on, 'reason', pause.reason) ORDER BY pause.starts_on DESC) FROM enrollment_pauses pause WHERE pause.enrollment_id = e.id), '[]') AS pauses FROM enrollments e JOIN courses c ON c.id = e.course_id JOIN users t ON t.id = c.teacher_id LEFT JOIN rooms r ON r.id = c.standard_room_id LEFT JOIN locations loc ON loc.id = r.location_id WHERE e.participant_id = ${participantId} ORDER BY e.active DESC, c.code`,
...
export async function PATCH(request: Request, context: { params: Promise<{ participantId: string }> }) {
  try {
    ...
    const updated = await sql.begin(async (transaction) => {
      ...
      const [user] = await transaction`UPDATE users SET name = COALESCE(${nextName ?? null}, name), email = COALESCE(${input.email?.toLowerCase() ?? null}, email), password_hash = COALESCE(${passwordHash}, password_hash) WHERE id = ${participantId} RETURNING id, name, email, role`;
      ...
      await transaction`INSERT INTO change_history (id, entity_type, entity_id, event_type, summary, before_data, after_data, actor_id) VALUES (${randomUUID()}, 'participant', ${participantId}, 'updated', 'Teilnehmerdaten geändert', ${JSON.stringify(existing)}, ${JSON.stringify(safeInput)}, ${actor.id})`;
      return { ...user, ...profile };
    });
