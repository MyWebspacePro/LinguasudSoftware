...
export async function GET(_request: Request, context: { params: Promise<{ courseId: string }> }) {
  try {
    ...
    const [schedules, lessons, enrollments, history, qualifications, breaks] = await Promise.all([
      ...
      sql`SELECT id, course_id, starts_on, ends_on, reason, created_by, created_at FROM course_breaks WHERE course_id = ${courseId} ORDER BY starts_on DESC`,
    ]);
    return NextResponse.json({ course: { ...course, schedules, lessons, enrollments, history, breaks, teacher: { ...course.teacher, qualifications } } });
  } catch (error) {
