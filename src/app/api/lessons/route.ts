...
export async function GET(request: Request) {
  try {
    ...
    const lessons = user.role === "office"
      ? await db()`SELECT lessons.*, courses.code, courses.language, courses.level, rooms.name AS room_name, locations.name AS location_name, users.name AS teacher_name, (SELECT count(*)::int FROM enrollments WHERE course_id = courses.id AND active = true) AS participant_count FROM lessons JOIN courses ON courses.id = lessons.course_id JOIN rooms ON rooms.id = lessons.room_id JOIN locations ON locations.id = rooms.location_id JOIN users ON users.id = lessons.teacher_id WHERE (${date}::date IS NULL OR (lessons.starts_at AT TIME ZONE 'Europe/Zurich')::date = ${date}::date) ORDER BY lessons.starts_at`
      : await db()`SELECT lessons.*, courses.code, courses.language, courses.level, rooms.name AS room_name, locations.name AS location_name, users.name AS teacher_name, (SELECT count(*)::int FROM enrollments WHERE course_id = courses.id AND active = true) AS participant_count FROM lessons JOIN courses ON courses.id = lessons.course_id JOIN rooms ON rooms.id = lessons.room_id JOIN locations ON locations.id = rooms.location_id JOIN users ON users.id = lessons.teacher_id WHERE lessons.teacher_id = ${user.id} AND (${date}::date IS NULL OR (lessons.starts_at AT TIME ZONE 'Europe/Zurich')::date = ${date}::date) ORDER BY lessons.starts_at`;
