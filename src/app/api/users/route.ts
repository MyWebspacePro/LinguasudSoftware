...
export async function GET(request: Request) {
  try {
    ...
    const users = role && ["office", "teacher", "participant"].includes(role)
      ? await db()`SELECT id, name, email, role, created_at FROM users WHERE role = ${role} ORDER BY name`
      : await db()`SELECT id, name, email, role, created_at FROM users ORDER BY role, name`;
