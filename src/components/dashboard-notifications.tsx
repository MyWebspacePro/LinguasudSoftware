...
export function DashboardNotifications({ onOpenCourse }: { onOpenCourse?: (courseId: string) => void } = {}) {
  ...
  return <section className="dashboard-notifications" aria-label="Offene Büro-Aufgaben">
    ...
    {tasks.length === 0 ? <p className="dashboard-notifications__empty">Keine offenen Aufgaben.</p> : <div className="dashboard-notifications__list">{tasks.map((task) => <article key={task.id}><span className="dashboard-notifications__badge">{taskTypeLabel(task.task_type)}</span><div><strong>{task.title}</strong><p>{task.course_id && task.course_code ? <button className="course-link" onClick={() => onOpenCourse?.(task.course_id as string)} type="button">{task.course_code}</button> : <b>{task.course_code ?? "Kurs"}</b>}{task.language || task.level ? ` · ${task.language ?? ""} ${task.level ?? ""}` : ""}</p><p>{task.description}</p><small>{task.actor_name ?? "System"} · {taskDateFormatter.format(new Date(task.created_at))}</small></div><button className="dashboard-notifications__task-action" disabled={savingTaskId === task.id} onClick={() => void completeTask(task.id)} type="button">{savingTaskId === task.id ? "…" : "Erledigt"}</button></article>)}</div>}
  </section>;
}
