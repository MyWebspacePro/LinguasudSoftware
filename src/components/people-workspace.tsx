  ...
  const load = useCallback(async () => {
    ...
    try {
      const [peopleResponse, coursesResponse, enrollmentsResponse] = await Promise.all([
        fetch(`/api/users?role=${role}`, { credentials: "same-origin" }),
  ...
  async function createPerson(formData: FormData) {
    ...
    try {
      const response = await fetch("/api/users", {
        ...
        body: JSON.stringify({ name: formData.get("name"), email: formData.get("email"), password: formData.get("password"), role }),
      });
