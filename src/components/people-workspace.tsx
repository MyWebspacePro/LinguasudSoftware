...
function TeacherCourseList({ courses, onOpenCourse }: { courses: Course[]; onOpenCourse?: (courseId: string) => void }) {
  if (!courses.length) return <p className="course-schedule-summary">Noch keinem Kurs zugeordnet</p>;
  return <div className="teacher-course-list" aria-label="Kurse der Lehrperson">{courses.map((course) => <button className="course-link" key={course.id} onClick={() => onOpenCourse?.(course.id)} type="button">{course.code} · {course.language} {course.level}</button>)}</div>;
}

export function PeopleWorkspace({ mode, onOpenCourse, focusPersonId = null }: { mode: "participants" | "teachers"; onOpenCourse?: (courseId: string) => void; focusPersonId?: string | null }) {
  ...
  useEffect(() => {
    if (!focusPersonId || isLoading || editingPerson?.id === focusPersonId) return;
    const person = people.find((item) => item.id === focusPersonId);
    if (!person) return;
    queueMicrotask(() => {
      setQualificationRows(rowsFromTeachingLevels(person.teaching_levels));
      setNoteDrafts([""]);
  ...
  }, [editingPerson?.id, focusPersonId, isLoading, people]);
