...
type Person = { id: string; name: string; email: string; role: PersonRole; phone?: string | null; street?: string | null; postal_code?: string | null; city?: string | null; date_of_birth?: string | null; preferred_contact?: "email" | "phone" | "postal" | null; emergency_name?: string | null; emergency_phone?: string | null; email_reminders?: boolean; language_preference?: string | null; notes?: string | null; teacher_code?: string | null; rate_per_lesson?: number | null; teaching_levels?: TeachingLevel[]; course_count?: number; history?: Array<{ eventType: string; summary: string; occurredAt: string }> };
...
function parseTeachingLevels(value: FormDataEntryValue | null): TeachingLevel[] {
  if (typeof value !== "string") return [];
  return value.split("\n").map((line) => {
    const [language, levels] = line.split(":", 2);
    return { language: (language ?? "").trim(), levels: (levels ?? "").split(",").map((level) => level.trim().toUpperCase()).filter(Boolean) };
  }).filter((item) => item.language.length >= 2 && item.levels.length > 0);
}

function formatTeachingLevels(value: TeachingLevel[] | undefined) {
  return (value ?? []).map((item) => `${item.language}: ${item.levels.join(", ")}`).join("\n");
}
...
export function PeopleWorkspace({ mode }: { mode: "participants" | "teachers" }) {
  ...
  async function createPerson(formData: FormData) {
    ...
    try {
      const response = await fetch(`/api/${role === "participant" ? "participants" : "teachers"}`, {
        ...
        body: JSON.stringify({ name: formData.get("name"), email: formData.get("email"), password: formData.get("password"), phone: formData.get("phone") || null, street: formData.get("street") || null, postalCode: formData.get("postalCode") || null, city: formData.get("city") || null, dateOfBirth: isParticipantMode ? formData.get("dateOfBirth") || null : undefined, preferredContact: isParticipantMode ? formData.get("preferredContact") : undefined, emergencyName: isParticipantMode ? formData.get("emergencyName") || null : undefined, emergencyPhone: isParticipantMode ? formData.get("emergencyPhone") || null : undefined, emailReminders: isParticipantMode ? formData.get("emailReminders") === "on" : undefined, languagePreference: isParticipantMode ? formData.get("languagePreference") || null : undefined, teacherCode: formData.get("teacherCode") || null, teachingLevels: isParticipantMode ? undefined : parseTeachingLevels(formData.get("teachingLevels")), ratePerLesson: formData.get("ratePerLesson") ? Number(formData.get("ratePerLesson")) : null, notes: formData.get("notes") || null }),
      });
  ...
  return <section className="management-preview" aria-labelledby="people-title">
    ...
    {isLoading ? <p className="planner-state">{title} werden geladen …</p> : <div className="preview-grid">{people.length === 0 ? <p className="planner-state">Noch keine {isParticipantMode ? "Teilnehmer" : "Lehrpersonen"} angelegt.</p> : people.map((person) => <article key={person.id}><span>{isParticipantMode ? "Teilnehmer" : "Lehrperson"}</span><h3>{person.name}</h3><p>{person.email}</p><p className="course-schedule-summary">{isParticipantMode ? (enrollmentsByParticipant.get(person.id) ?? []).map((enrollment) => `${enrollment.course_code} · ${enrollment.billing_type === "private" ? `${enrollment.credit_lessons ?? 0} Lektionen` : "Kostenträger"}`).join(" · ") || "Noch in keinem Kurs" : <>{(coursesByTeacher.get(person.id) ?? []).map((course) => `${course.code} · ${course.language} ${course.level}`).join(" · ") || "Noch keinem Kurs zugeordnet"}{person.teaching_levels?.length ? <><br />Unterrichtet: {person.teaching_levels.map((item) => `${item.language} ${item.levels.join(", ")}`).join(" · ")}</> : null}</>}</p><button className="quiet-button" onClick={() => setEditingPerson(person)} type="button">Stammdaten bearbeiten</button></article>)}</div>}
    {personDialogOpen ? <div className="dialog-backdrop" role="presentation"><form action={createPerson} className="attendance-dialog" aria-labelledby="create-person-title"><button aria-label="Personenformular schliessen" className="dialog-close" onClick={() => setPersonDialogOpen(false)} type="button">×</button><p className="eyebrow">Büro</p><h2 id="create-person-title">{isParticipantMode ? "Teilnehmer anlegen" : "Lehrperson anlegen"}</h2><div className="form-grid"><label>Name<input name="name" required /></label><label>E-Mail<input name="email" required type="email" /></label><label>Telefon<input name="phone" /></label><label>Strasse<input name="street" /></label><label>PLZ<input name="postalCode" /></label><label>Ort<input name="city" /></label>{isParticipantMode ? <><label>Geburtsdatum<input name="dateOfBirth" type="date" /></label><label>Bevorzugter Kontakt<select defaultValue="email" name="preferredContact"><option value="email">E-Mail</option><option value="phone">Telefon</option><option value="postal">Post</option></select></label><label>Notfallkontakt<input name="emergencyName" placeholder="Name" /></label><label>Notfalltelefon<input name="emergencyPhone" /></label><label>Bevorzugte Sprache<input name="languagePreference" placeholder="z. B. Deutsch" /></label><label className="checkbox-field"><input name="emailReminders" type="checkbox" />E-Mail-Erinnerungen erlauben</label></> : <><label>Lehrpersonenkürzel<input name="teacherCode" /></label><label>Tarif pro Lektion<input min="0" name="ratePerLesson" step="0.05" type="number" /></label><label className="form-grid__full">Unterrichtete Sprachen und Niveaus<textarea name="teachingLevels" placeholder={'Deutsch: A0, A1, A2\nEnglisch: B1, C1'} rows={4} /><small>Eine Zeile pro Sprache, Niveaus mit Komma trennen.</small></label></>}<label className="form-grid__full">Notizen<textarea name="notes" rows={3} /></label><label>Startpasswort<input minLength={12} name="password" required type="password" /></label></div><div className="dialog-actions"><button className="quiet-button" onClick={() => setPersonDialogOpen(false)} type="button">Abbrechen</button><button className="primary-button" disabled={isCreating} type="submit">{isCreating ? "Wird angelegt …" : "Zugang anlegen"}</button></div></form></div> : null}
    {editingPerson ? <div className="dialog-backdrop" role="presentation"><form action={updatePerson} className="attendance-dialog" aria-labelledby="edit-person-title"><button aria-label="Bearbeitung schliessen" className="dialog-close" onClick={() => setEditingPerson(null)} type="button">×</button><p className="eyebrow">Büro</p><h2 id="edit-person-title">{editingPerson.name} bearbeiten</h2><div className="form-grid"><label>Name<input defaultValue={editingPerson.name} name="name" required /></label><label>E-Mail<input defaultValue={editingPerson.email} name="email" required type="email" /></label><label>Telefon<input defaultValue={editingPerson.phone ?? ""} name="phone" /></label><label>Strasse<input defaultValue={editingPerson.street ?? ""} name="street" /></label><label>PLZ<input defaultValue={editingPerson.postal_code ?? ""} name="postalCode" /></label><label>Ort<input defaultValue={editingPerson.city ?? ""} name="city" /></label>{isParticipantMode ? <><label>Geburtsdatum<input defaultValue={editingPerson.date_of_birth ?? ""} name="dateOfBirth" type="date" /></label><label>Bevorzugter Kontakt<select defaultValue={editingPerson.preferred_contact ?? "email"} name="preferredContact"><option value="email">E-Mail</option><option value="phone">Telefon</option><option value="postal">Post</option></select></label><label>Notfallkontakt<input defaultValue={editingPerson.emergency_name ?? ""} name="emergencyName" /></label><label>Notfalltelefon<input defaultValue={editingPerson.emergency_phone ?? ""} name="emergencyPhone" /></label><label>Bevorzugte Sprache<input defaultValue={editingPerson.language_preference ?? ""} name="languagePreference" /></label><label className="checkbox-field"><input defaultChecked={editingPerson.email_reminders ?? false} name="emailReminders" type="checkbox" />E-Mail-Erinnerungen erlauben</label></> : <><label>Lehrpersonenkürzel<input defaultValue={editingPerson.teacher_code ?? ""} name="teacherCode" /></label><label>Tarif pro Lektion<input defaultValue={editingPerson.rate_per_lesson ?? ""} min="0" name="ratePerLesson" step="0.05" type="number" /></label><label className="form-grid__full">Unterrichtete Sprachen und Niveaus<textarea defaultValue={formatTeachingLevels(editingPerson.teaching_levels)} name="teachingLevels" rows={4} /><small>Eine Zeile pro Sprache, Niveaus mit Komma trennen.</small></label></>}<label className="form-grid__full">Notizen<textarea defaultValue={editingPerson.notes ?? ""} name="notes" rows={4} /></label></div>{editingPerson.history?.length ? <div className="history-list"><strong>Änderungshistorie</strong>{editingPerson.history.slice(0, 8).map((event, index) => <p key={`${event.occurredAt}-${index}`}><b>{event.summary}</b><span>{new Intl.DateTimeFormat("de-CH", { dateStyle: "medium", timeStyle: "short" }).format(new Date(event.occurredAt))}</span></p>)}</div> : <p className="planner-state">Noch keine Änderungen protokolliert.</p>}<div className="dialog-actions"><button className="quiet-button" onClick={() => setEditingPerson(null)} type="button">Abbrechen</button><button className="primary-button" disabled={isCreating} type="submit">{isCreating ? "Wird gespeichert …" : "Daten speichern"}</button></div></form></div> : null}
