...
export function PeopleWorkspace({ mode, onOpenCourse }: { mode: "participants" | "teachers"; onOpenCourse?: (courseId: string) => void }) {
  ...
  return <section className="management-preview" aria-labelledby="people-title">
    ...
    {isLoading ? <p className="planner-state">{title} werden geladen …</p> : <div className="preview-grid">{people.length === 0 ? <p className="planner-state">Noch keine {isParticipantMode ? "Teilnehmer" : "Lehrpersonen"} angelegt.</p> : people.map((person) => <article key={person.id}><span>{isParticipantMode ? "Teilnehmer" : "Lehrperson"}</span><h3>{person.name}</h3><p>{person.email}</p>{isParticipantMode ? <ParticipantEnrollmentList enrollments={enrollmentsByParticipant.get(person.id) ?? []} onAddPause={(enrollment) => setPauseEnrollment(enrollment)} onEditBilling={(enrollment) => setEditingEnrollment(enrollment)} onOpenCourse={onOpenCourse} onRemovePause={(enrollment, pause) => void removePause(enrollment, pause)} /> : <p className="course-schedule-summary">{(coursesByTeacher.get(person.id) ?? []).map((course) => `${course.code} · ${course.language} ${course.level}`).join(" · ") || "Noch keinem Kurs zugeordnet"}{person.teaching_levels?.length ? <><br />Unterrichtet: {person.teaching_levels.map((item) => `${item.language} ${item.levels.join(", ")}`).join(" · ")}</> : null}</p>}<button className="quiet-button" onClick={() => setEditingPerson(person)} type="button">Stammdaten bearbeiten</button></article>)}</div>}
